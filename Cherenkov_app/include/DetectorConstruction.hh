...
The header load the detector geometry (construction) and provides
templates for easier loading of material properties from a file.

1. Detector geometry (construction):
The detector is very simple and consists of a world volume, containing
a very thin radiator and a small-aperture detector according to the
experiment to generate quasi-monochromatic Cherenkov spectral lines.
For more information, check the ReamMe file of the example and follow
the DOI links provided there.

2. Templates to load material properties:
Usually, it's the best practice to load data from external files and
not include them directly into the code. That way, it is much easier
to modify and load new data. The provided functions allow one to easily
do this.
*/
...
private:
  ...
  std::unique_ptr<G4VisAttributes> fVisAttrHide;
  DetectorConstruction_Messenger* fDetConstructionMessenger;
  //private variables
  G4String fRadiatorMaterialName;
  G4double fRadiatorAngle;
  G4double fRadiatorThickness; // half-thickness
  ...
  const G4double enUnit, const char separator, Args... args) -> std::enable_if_t<(std::is_same_v<const char*, Args> && ...)>
{
  ...
  if (!found_Energy_arg) {
    theError <<
      "You must use \"Energy\" argument to load material properties through this function\n"
      "The code is proceeding without any material properties loaded\n";
  ...
  while (true) {
    if (iFS.eof()) {
      theError << "The material" << std::quoted(theMat->GetName()) <<
        "hasn't been found in the input file\n"
        "The code is proceeding without any material properties loaded\n";
  ...
  while (true) {
    ...
    if (line.empty() || line.find("Data end") != line.npos || iFS.eof()) {
      ...
      if (theData[helperVec[0]].size() < 2) {
        theError <<
          "Too few data to use them for material properties vectors...\n"
          "The code is proceeding without any material properties loaded\n";
    ...
    for (const char* i : helperVec) {
      if (iSS.peek() == EOF) {
        theError << "The data is missing - each line should have " << sizeof...(args) <<
      ...
      try {
        ...
        theError <<
          "The data cannot be converted to double type.\n"
          "Successfully loaded data are:\n";
        PrintLoadedData();
        theError << "\nThe line:\n\"" << line << "\"\ncannot be converted\n";
    ...
    if (iSS.peek() != EOF) {
      theError << "An excess of the data - each line should have " << sizeof...(args)
        << " double values\nSuccessfully loaded data are:\n";
      PrintLoadedData();
      theError << "\t\nThe line:\n\"" << line << "\"\nhas a data excess\n\n";
