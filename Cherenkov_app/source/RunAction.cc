...
void RunAction::EndOfRunAction(const G4Run*)
{
  ...
  if (isMaster) {
    ...
    /*
    care here, 'current_path' function works as long as the ChR_app_exe
    is directly executed. If one however executes it from another project,
    the executable path will most likely fail and the path to the executable
    will not be correct (there is no such a function in the std:: namespace,
    but one can easily write it if needed).
    */
