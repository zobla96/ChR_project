...
m_deltaPhi(180.), m_thetaMin(-DBL_MAX), m_thetaMax(DBL_MAX), m_withGaussSigma(false) {
...
G4ClassificationOfNewTrack StackingAction::ClassifyNewTrack(const G4Track* aTrack) {
	...
	*/
	...
	G4double newPhi = (90 + (G4UniformRand() - 0.5) * m_deltaPhi) * deg;
	G4double sinNewPhi = std::sin(newPhi);
	G4double cosNewPhi = std::cos(newPhi);
	if (!m_withGaussSigma) {
		G4double cosTheta = parentData->m_globalDirection.dot(nonConstPhotonDirection);
	...
	G4double sampledRI = p_rindexVector->Value(aTrack->GetTotalEnergy());
	...
	G4double theAngleCos = parentData->m_globalDirection.dot(m_rotationMatrix(G4ThreeVector{ 0., 0., 1. }));
	if (theAngleCos == 0.)
		return fKill;
	G4double thetaChR = std::acos(1. / (sampledRI * parentData->m_beta));
	// NOTE
