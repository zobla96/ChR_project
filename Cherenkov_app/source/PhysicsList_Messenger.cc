...
:p_physicsList(phList) {
	...
	p_selectOpticalPhysics = new G4UIcmdWithAnInteger{ "/ChR_project/PhysicsList/selectOpticalPhysics", this };
	p_selectOpticalPhysics->SetGuidance("Used to change optical physics into:");
	p_selectOpticalPhysics->SetGuidance("0 - G4OpticalPhysics\n1 - G4OpticalPhysics_option1\n2 - G4OpticalPhysics_option2");
	p_selectOpticalPhysics->SetParameterName("opticalPhysID", false);
	p_selectOpticalPhysics->SetRange("opticalPhysID>=0 && opticalPhysID<3");
	p_selectOpticalPhysics->SetToBeBroadcasted(false);
	p_selectOpticalPhysics->AvailableForStates(G4State_PreInit);
}
...
void PhysicsList_Messenger::SetNewValue(G4UIcommand* uiCmd, G4String aStr) {
	if (uiCmd == p_gammaRangeCut) {
		...
		G4int emIdNo = p_selectEmPhysics->ConvertToInt(aStr);
		switch (emIdNo) {
		case 0:
	...
	else if (uiCmd == p_selectOpticalPhysics) {
		G4int opticalID = p_selectOpticalPhysics->ConvertToInt(aStr);
		switch (opticalID) {
		case 0:
			p_physicsList->ChangeOpticalPhysics(UseOptical::G4OpticalPhysics);
			break;
		case 1:
			p_physicsList->ChangeOpticalPhysics(UseOptical::G4OpticalPhysics_option1);
			break;
		case 2:
			p_physicsList->ChangeOpticalPhysics(UseOptical::G4OpticalPhysics_option2);
			break;
		default: // shouldn't happen
