...
G4VParticleChange* G4ThinTargetChR_Model::PostStepModelDoIt(const G4Track& aTrack, const G4Step& aStep, const G4CherenkovMatData& aChRMatData)
{
  ...
  if (aChRMatData.fMinAxis > 2) {
    const char* err =
      "Currently, G4ThinTargetChR_Model can be used only for G4Tubs and G4Box\n"
  ...
  if (aChRMatData.fMiddlePoint) {
    aLocalMiddlePoint = *aChRMatData.fMiddlePoint;
  ...
  if (meanNumberOfPhotons <= 0.0) {
    //unchanged particle; number of secondaries is 0 anyway after fParticleChange->Initialize(aTrack);
    return fParticleChange;
  }
  ...
  fParticleChange->SetNumberOfSecondaries(noOfPhotons);
  ...
  if (aChRMatData.fExoticRIndex) {
    if (beta > fChRPhysDataVec[materialID].fAroundBetaValues.back().fBetaValue) { // *1*
      bigBetaCDFVector = new std::vector<std::pair<G4double, G4double>>{};
      std::vector<G4ThreeVector>* bigBetaVec = fChRPhysDataVec[materialID].fBigBetaCDFVector;
      ...
      bigBetaCDFVector = physDataVec[static_cast<size_t>(std::lower_bound(physDataVec.begin() + 1, physDataVec.end(), beta,
        [](const G4ChRPhysTableData::G4AroundBetaBasedValues& value1, const G4double value2) {return value1.fBetaValue < value2; }) - physDataVec.begin())].fValuesCDF;
    }
  }
  else {
    ...
    if (aChRMatData.fExoticRIndex) {
      ...
      while (true) {
        ...
        if (sin2Theta <= 0.) {
          ...
          if (!aChRMatData.fExoticFlagInital)
  ...
  if (fUseModelWithEnergyLoss) {
    ...
    fParticleChange->ProposeEnergy(fParticleChange->GetEnergy() - lossEnergy);
  }
  if (fVerboseLevel > 1)
    std::cout << "\n Exiting from G4ThinTargetChR_Model::PostStepModelDoIt -- NumberOfSecondaries = "
    << fParticleChange->GetNumberOfSecondaries() << G4endl;

  if (bigBetaCDFVector && beta > fChRPhysDataVec[materialID].fAroundBetaValues.back().fBetaValue)
...
void G4ThinTargetChR_Model::BuildModelPhysicsTable(const G4ParticleDefinition& aParticleDef)
{
  ...
  for (auto thisIterator = thePhysStore->begin(); thisIterator < thePhysStore->end(); thisIterator++) {
    ...
    if (matData.fMinAxis != 255) // already checked and finished the job - no need to do it again
      continue;
    if (thisLogicVolume->GetNoDaughters() > 0) {
      matData.fMinAxis = 254;
      if (fVerboseLevel > 0) {
    ...
    if (typeid(*aSolid) == typeid(G4Box)) {
      ...
      */
      ...
      if (returnFlags < 0x40 && !GetBit(returnFlags, 0) && !GetBit(returnFlags, 1) && !GetBit(returnFlags, 2)) {
        ...
        if (matData.fMinAxis == 254)
          continue;
        //and now I finally need to find the point to the middle of the volume
        delete matData.fMiddlePoint; // just in case of some bug, but this should always be a nullptr
        matData.fMiddlePoint = new G4ThreeVector{
        ...
        aTransform.ApplyPointTransform(*matData.fMiddlePoint); //this is the critical point if using active transformations
        if (fVerboseLevel > 0)
          PrintSuccessInfo(thisLogicVolume, matData, &dimensionNoOfLayers);
        continue;
      }
      ...
      if (counter == 1) {
        matData.fHalfThickness = halfThickness;
        matData.fMinAxis = 2;
        if (fVerboseLevel > 0)
          PrintSuccessInfo(thisLogicVolume, matData);
        continue;
      ...
      */
      ...
      if (!breakInLayers && foundGluedLayers) {
        ...
        matData.fHalfThickness = (G4double)counter * halfThickness;
        delete matData.fMiddlePoint; // just in case of some bug, but this should always be a nullptr
        ...
        matData.fMiddlePoint =
          new G4ThreeVector{ 0., 0., zBuffer.front().first.getZ() + matData.fHalfThickness - halfThickness };
        aTransform.ApplyPointTransform(*matData.fMiddlePoint); //this is the critical point if using active transformations
        matData.fMinAxis = 2;
        if (fVerboseLevel > 0) {
  ...
  const G4CherenkovMatData& aChRMatData) const
{
  ...
  if (aChRMatData.fMinAxis == 0) {
    ...
    intensity = (-aChRMatData.fHalfThickness + localMiddlePoint.getX() - localPrePoint.getX()) / localDirection.getX();
    localEntryPoint.set(-aChRMatData.fHalfThickness + localMiddlePoint.getX(),
      localPrePoint.getY() + intensity * localDirection.getY(),
      localPrePoint.getZ() + intensity * localDirection.getZ());
    intensity = (aChRMatData.fHalfThickness + localMiddlePoint.getX() - localPrePoint.getX()) / localDirection.getX();
    localExitPoint.set(aChRMatData.fHalfThickness + localMiddlePoint.getX(),
      localPrePoint.getY() + intensity * localDirection.getY(),
      localPrePoint.getZ() + intensity * localDirection.getZ());
    if (localDirection.getX() < 0)
      std::swap(localEntryPoint, localExitPoint);
  } // end of "aChRMatData.fMinAxis == 0"
  else if (aChRMatData.fMinAxis == 1) {
    ...
    intensity = (-aChRMatData.fHalfThickness + localMiddlePoint.getY() - localPrePoint.getY()) / localDirection.getY();
    localEntryPoint.set(localPrePoint.getX() + intensity * localDirection.getX(),
      -aChRMatData.fHalfThickness + localMiddlePoint.getY(),
      localPrePoint.getZ() + intensity * localDirection.getZ());
    intensity = (aChRMatData.fHalfThickness + localMiddlePoint.getY() - localPrePoint.getY()) / localDirection.getY();
    ...
    intensity = (-aChRMatData.fHalfThickness + localMiddlePoint.getZ() - localPrePoint.getZ()) / localDirection.getZ();
    localEntryPoint.set(localPrePoint.getX() + intensity * localDirection.getX(),
      localPrePoint.getY() + intensity * localDirection.getY(),
      -aChRMatData.fHalfThickness + localMiddlePoint.getZ());
    intensity = (aChRMatData.fHalfThickness + localMiddlePoint.getZ() - localPrePoint.getZ()) / localDirection.getZ();
    localExitPoint.set(localPrePoint.getX() + intensity * localDirection.getX(),
      localPrePoint.getY() + intensity * localDirection.getY(),
      aChRMatData.fHalfThickness + localMiddlePoint.getZ());
  ...
  const G4CherenkovMatData& matData) const
{
  ...
  if (matData.fMinAxis == 0) {
    if (photonDirection.getX() == 0.)
      return DBL_MAX;
    // 2 * matData.fHalfThickness == localExitPoint.getX() - localEntryPoint.getX()
    G4double intensity = 2 * matData.fHalfThickness / photonDirection.getX();
  ...
  else if (matData.fMinAxis == 1) {
    if (photonDirection.getY() == 0.)
      return DBL_MAX;
    // 2 * matData.fHalfThickness == localExitPoint.getY() - localEntryPoint.getY()
    G4double intensity = 2 * matData.fHalfThickness / photonDirection.getY();
    G4ThreeVector exitPhotonPoint =
      G4ThreeVector{
    ...
    if (photonDirection.getZ() == 0.)
      return DBL_MAX;
    // 2 * matData.fHalfThickness == localExitPoint.getZ() - localEntryPoint.getZ()
    G4double intensity = 2 * matData.fHalfThickness / photonDirection.getZ();
    G4ThreeVector exitPhotonPoint =
      G4ThreeVector{
          localEntryPoint.getX() + intensity * photonDirection.getX(),
          localEntryPoint.getY() + intensity * photonDirection.getY(),
...
void G4ThinTargetChR_Model::SetBoxPhysicsTableParameters(G4CherenkovMatData& matData, const G4ThreeVector& halfThickness) const
{
  ...
  if (halfThickness.getZ() < minSize) {
    ...
    singleMinimal = false;
  if (!singleMinimal) {
    if (fVerboseLevel > 0 && !GetBit(o_staticFlag, 5)) {
  ...
  matData.fHalfThickness = minSize;
}
