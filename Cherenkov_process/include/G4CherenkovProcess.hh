...
protected:
  ...
  std::vector<G4BaseChR_Model*> fRegisteredModels;
private:
  G4CherenkovMatData* fSelectedMatData = nullptr;
  G4BaseChR_Model* fSelectedModel = nullptr;
  G4CherenkovProcess_Messenger* fChRProcessMessenger;
};
...
G4double G4CherenkovProcess::PostStepGetPhysicalInteractionLength(const G4Track& aTrack, G4double previousStepSize, G4ForceCondition* aForceCondition)
{
  try {
    ...
    // might need to add 'if (fSelectedMatData) throw ...'
    fSelectedMatData = &G4ExtraOpticalParameters::GetInstance()->FindOrCreateChRMatData(aTrack.GetVolume()->GetLogicalVolume());
    fSelectedModel = fRegisteredModels.at(fSelectedMatData->m_executeModel);
    return fSelectedModel->PostStepModelIntLength(aTrack, previousStepSize, aForceCondition);
  }
...
{
  return fSelectedModel->PostStepModelDoIt(aTrack, aStep, *fSelectedMatData);
}
...
{
  for (auto* aModel : fRegisteredModels)
...
{
  for (G4BaseChR_Model* aRegisteredModel : fRegisteredModels) {
  ...
  fRegisteredModels.push_back(aModel);
  return true;
}
...
{
  try {
    return fRegisteredModels.at(anID);
  }
  catch (std::out_of_range) {
    std::ostringstream err;
