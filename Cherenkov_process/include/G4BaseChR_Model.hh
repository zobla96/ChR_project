...
5. p_bigBetaCDFVector -
         the value is mostly 'nullptr'. It is used for materials
...
1. fNoOfBetaSteps -
        for filling the beta-values vector from betaMin to
        betaMax with "m_noOfBetaStep + 1" nodes
2. fIncludeFiniteThickness -
        Cherenkov models should mark if they can
        consider thin radiators adequately (currently not used,
        but it could be used for some safety conditions)
3. fUseModelWithEnergyLoss -
        set to true to include conservation energy law
...
protected:
  ...
  const char* fChRModelName;
  unsigned char fVerboseLevel;
  G4bool fIncludeFiniteThickness;
  G4bool fUseModelWithEnergyLoss;
...
{
  fUseModelWithEnergyLoss = value;
}

//=======Get inlines=======
const char* G4BaseChR_Model::GetChRModelName() const
{
  return fChRModelName;
}
