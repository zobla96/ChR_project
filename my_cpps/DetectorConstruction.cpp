...
DetectorConstruction::DetectorConstruction() //defining variables here for faster re-compiling
:m_radiatorAngle(22.0_deg),
m_radiatorThickness(100._um),
m_noOfRadLayers(1),
m_radiatorMaterial("G4_SILICON_DIOXIDE"),
...
void DetectorConstruction::LoadMaterials() {
	...
	nist->FindOrBuildMaterial("G4_SILICON_DIOXIDE"); //we use this one as quartz (not completely correct, but good enough)
	nist->BuildMaterialWithNewDensity("Diamond", "G4_C", 3.5 * g / cm3);
	nist->BuildMaterialWithNewDensity("Low_density_air", "G4_AIR", airPres * 1.e-9);
	//just need some low value pressure to minimize interaction, the value is not important
	
	//adding material properties
	HelperToBuildMatPropTable obj{};
	obj.FillMatPropData(G4Material::GetMaterial("Low_density_air"), "RINDEX", [](double) {return 1.; }, 1.458637_eV, 4.13280_eV, 2);
	obj.FillMatPropData(G4Material::GetMaterial("G4_SILICON_DIOXIDE"), "ABSLENGTH", [](double) {return 0.5_m; }, 1.458637_eV, 4.13280_eV, 2);
	obj.FillMatPropertiesVectors("refractive_index_data.txt", G4Material::GetMaterial("G4_SILICON_DIOXIDE"), EnergyValue::Wavelength, 1._um, '\t', "Energy", "RINDEX");
	obj.FillMatPropertiesVectors("refractive_index_data.txt", G4Material::GetMaterial("Diamond"), EnergyValue::Wavelength, 1._um, '\t', "Energy", "RINDEX");
}
...
void DetectorConstruction::LoadRadiator() {
	...
	G4LogicalVolume* radiatorLogic = new G4LogicalVolume{ radiatorSolid, G4Material::GetMaterial(m_radiatorMaterial), "radiatorLogic" };
	auto myParameters = MyOpticalParameters<ChRModelIndex>::GetInstance();
	myParameters->AddNewChRMatData(radiatorLogic, CherenkovMatData<ChRModelIndex>{ChRModelIndex::TammThinTargetChR, 2 * m_radiatorThickness});
	...
	if (m_noOfRadLayers > 1) {
		...
		double layerThickness = m_radiatorThickness / /*(double)*/m_noOfRadLayers;
		G4Tubs* radLayerSolid = new G4Tubs{ "radiatorLayerSolid", 0., 3._cm, layerThickness, 0., 360._deg };
		G4LogicalVolume* radLayerLogic = new G4LogicalVolume{ radLayerSolid, G4Material::GetMaterial(m_radiatorMaterial), "radiatorLayerLogic" };
		radLayerLogic->SetVisAttributes(m_visAttrHide.get());
		myParameters->AddNewChRMatData(radLayerLogic, CherenkovMatData<ChRModelIndex>{ChRModelIndex::TammThinTargetChR, 2 * m_radiatorThickness});
...
void DetectorConstruction::LoadDetector() const {
	...
	/*
	for detectorSolid, the material is not important because we consider efficiency of 100%.
	Only later in data processing we include efficiency
	NOTE: if the material doesn't have Material Properties defined (e.g., G4_Air), opticalphotons won't be able to
	enter the volume and they will be killed! However, G4StepStatus::fGeomBoundary can be used to extract the
	information (that's done in SteppingAction::UserSteppingAction in this project)... That's because we consider
