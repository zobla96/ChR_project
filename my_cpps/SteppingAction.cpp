...
: p_trackingAction(trAction), p_eventAction(evAction), m_verboseLevel(val),
...
r_noOfPrimaries(PrimaryGeneratorAction::GetInstance()->GetRefNoOfParticles()) {
	...
	if (!detPhys) {
		G4Exception("SteppingAction::SteppingAction()", "FE1020", FatalException, "G4VPhysicalVolume instance \"detectorPhys\" not found!\n");
...
void SteppingAction::UserSteppingAction(const G4Step* aStep) {
	...
	//NOTE: the following if is made as an example of something similar to what we used in the past
	//to numerically integrate PCM + ionization losses for heavy ions (the paper is in press currently)
	if (r_noOfRadLayers > 1 && aTrack->GetTrackID() <= r_noOfPrimaries) {
		...
		if (aStep->GetPostStepPoint()->GetStepStatus() == fGeomBoundary) {
			if (aStep->GetPostStepPoint()->GetPhysicalVolume()->GetName() == "radiatorLayerPhys") {
				if (aStep->GetPostStepPoint()->GetMomentumDirection().getZ() < 0) {
					G4Exception("SteppingAction::UserSteppingAction", "WE1011", JustWarning, "A primary moving backwards!\n");
					//this should never happen for the used energies, I believe
					aTrack->SetTrackStatus(fStopAndKill);
					return;
				}
				//no need for safeties for our energies
				p_eventAction->AddToLayerDataVec(prePV->GetCopyNo(), p_trackingAction->GetLayerDeltaEnergy(aTrack), p_trackingAction->GetLayerStep(aTrack));
	...
	if (aStep->GetPostStepPoint()->GetStepStatus() == fGeomBoundary) {
		if (aStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "worldPhys" &&
			...
			if (aTrack->GetParticleDefinition()->GetParticleName() != "opticalphoton") return;
			...
			if (localCoords.getZ() > 1.e-10) { //CARE HERE! IEEE754 standard; don't care about negative values here
				if (m_verboseLevel > 1) {
					...
					G4Exception("SteppingAction::UserSteppingAction(...)", "WE1012", JustWarning, err);
				}
			}
			else {
