...
:p_physicsList(phList){
	...
	p_physListDir->SetGuidance("All commands related to physics list");

	p_useNonDefaultCuts = new G4UIcmdWithABool{ "/ChR_project/PhysicsList/useNonDefaultCuts", this };
	p_useNonDefaultCuts->SetGuidance("Used to change the PhysicsList::m_useNonDefaultCuts flag.");
	p_useNonDefaultCuts->SetGuidance("If this flag is set to 'true', non-default radiator cuts are used.");
	p_useNonDefaultCuts->SetGuidance("'False' condition means default cuts will be used.");
	p_useNonDefaultCuts->SetParameterName("useCutsFlag", true);
	p_useNonDefaultCuts->SetDefaultValue(true);
	p_useNonDefaultCuts->AvailableForStates(G4State_PreInit);
	...
	p_selectEmPhysics->SetParameterName("EmPhysicsNoFromTheList", true);
	p_selectEmPhysics->SetDefaultValue(0);
...
void PhysicsList_Messenger::SetNewValue(G4UIcommand* uiCmd, G4String aStr) {
	if (uiCmd == p_useNonDefaultCuts) {
		p_physicsList->SetUseNonDefaultCuts(p_useNonDefaultCuts->ConvertToBool(aStr));
	}
	else if (uiCmd == p_gammaRangeCut) {
		...
		default: //shouldn't happen
			G4Exception("PhysicsList_Messenger::SetNewValue", "WE1017", JustWarning, "Somehow, selected physics not found!\n");
			break;
		}
	}
	else
		G4Exception("PhysicsList_Messenger::SetNewValue", "WE1018", JustWarning, "Command not found!\n");
}
